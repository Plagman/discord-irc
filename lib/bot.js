...
class Bot {
  ...
  sendToDiscord(author, channel, text) {
    ...
    logger.debug('<irc> Sending message to Discord', withMentions, channel, '->', `#${discordChannel.name}`);
    discordChannel.send(withAuthor);
  }

  formatDirectMessage(author, text) {
    const withFormat = formatFromIRCToDiscord(text);
    ...
    this.discord.guilds
      ...
      .forEach((gm) => {
        if (this.ircClients[gm.user.id] == null) {
          ...
          if (i >= (this.goldenGate.maxIrcConnections || 3)) return;
          i += 1;
          ...
          ircUser
            ...
            .on('message', (who, where, what) => {
              if (where !== ircUser.nick) return;

              const content = this.formatDirectMessage(who, what);

              const to = this.findDiscordGuildMember(ircUser.nick);


              to.send(content);
            })
            .on('error', (error) => {
              logger.error(`<irc> <${ircUser.nick}> got error : ${error.args[0]} ${error.args[1]} ${error.args[2]}`);
              const to = this.findDiscordGuildMember(error.args[0]);
              to.send(`*${error.args[1]} ${error.args[2]}*`);
            });
        }
      });
  }

  findDiscordGuildMember(nick) {
    const allGM = this.discord.guilds
      .map(guild => guild.members)
      .reduce((users, it) => users.concat(it));

    const to = allGM.filter(gmem => gmem.displayName === nick).first();
    return to;
  }
  ...
  sendFromClient(who, ircChannel, message) {
    ...
    if (ircChannel) {
      ...
      let match = nickMessageRegEx.exec(message.content);
      let sent = false;
      while (match) {
        if (match.length !== 3) {
          message.author.send('*could not send pm.*');
        } else {
          const nick = match[1];
          const msg = match[2];
        ...
        match = nickMessageRegEx.exec(message.content);
      }
      if (!sent) {
        message.author.send('*no pm sent.*');
      }
    }
  }
